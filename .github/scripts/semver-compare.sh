...
# TODO: added for backwards-compatibility for template-sync, remove in 0.20.0
printf "\033[1;33m[WARN]\033[0mThis script will be deprecated in 0.20.0\n"
...
compare() {
  ...
  if ((maj1 != maj2)); then
    ((maj1 < maj2)) && echo "lt" || echo "gt"
    return
  fi
  if ((min1 != min2)); then
    ((min1 < min2)) && echo "lt" || echo "gt"
    return
  fi
  if ((pat1 != pat2)); then
    ((pat1 < pat2)) && echo "lt" || echo "gt"
    return
  fi
  ...
  for ((i = 0; ; i++)); do
    ...
    if ((i >= ${#ids1[@]})); then
      echo "lt"
      return
    fi # fewer fields → lower precedence
    if ((i >= ${#ids2[@]})); then
      echo "gt"
      return
    fi
    ...
    if $a_num && $b_num; then
      if ((10#$a != 10#$b)); then
        ((10#$a < 10#$b)) && echo "lt" || echo "gt"
        return
      fi
    elif $a_num; then
      echo "lt"
      return # §11.4.3: numeric < alphanumeric
    elif $b_num; then
      echo "gt"
      return
    else
