...
**`TOOLBOX_PLUGIN_ROOT` — session-wide runtime alternative.** A `SessionStart` hook in `klaude-plugin/hooks/hooks.json` exports `TOOLBOX_PLUGIN_ROOT` into every Bash call via `$CLAUDE_ENV_FILE`. Unlike `${CLAUDE_PLUGIN_ROOT}` (plugin-load-time only), `$TOOLBOX_PLUGIN_ROOT` is available in the shell environment throughout the session. Use it in Bash commands when the plugin root is needed at runtime — e.g., `ls $TOOLBOX_PLUGIN_ROOT/profiles/`. The hook receives the harness-substituted `${CLAUDE_PLUGIN_ROOT}` as its argument; a standalone resolver (`.claude/toolbox/scripts/cpr.py`) serves as fallback.
