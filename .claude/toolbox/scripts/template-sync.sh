...
fetch_upstream_templates() {
  ...
  if [[ -d "$upstream_root/.claude" ]]; then
    ...
    # Initial-setup-only scripts — never synced as managed files
    rm -f "$FETCHED_TEMPLATES_PATH/claude/toolbox/scripts/template-cleanup.sh"
    rm -f "$FETCHED_TEMPLATES_PATH/claude/toolbox/scripts/bootstrap.sh"
  fi
