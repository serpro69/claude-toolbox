...
> **Inline example.** Claim: "`billing` must not depend on the order schema module" (`present`, prohibitive, anchor = `billing` service). Resolve the anchor: confirm `services/billing/` exists. Then use the **Grep tool** (use the native equivalents of the Grep/Glob/Read operations) with pattern `orders/schema` scoped to path `services/billing/` (it covers `go.mod` and the `*.go` imports). A hit (e.g. `import "github.com/acme/orders/schema"`) is a forbidden static edge → `violated`; no hit → `verified` — prohibitive polarity, so the *absence* of the forbidden edge is the passing case.
