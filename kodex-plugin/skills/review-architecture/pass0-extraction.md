...
2. **Recall — "did it miss a claim?"** Intrinsically extrinsic.
   - *In evals:* gradeable against a structured `gold-claims.json` in the eval's `test-files/`; the `assertions[]` bullets reference gold entries ("claim G3 is extracted"), so each expected claim is graded PASS/FAIL individually — no free-form set arithmetic.
