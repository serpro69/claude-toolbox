...
- [finops-checklist.md](finops-checklist.md) — observability vendor cost traps (native vs generic check types, Prometheus auto-discovery leaks, metric include lists, per-environment controls), resource right-sizing signals, autoscaling hygiene, storage lifecycle, namespace cost attribution, idle/orphaned resource detection.
  **Load if:** the diff contains any file with observability agent configuration (Datadog `values.yaml` / `values-*.yaml` with `datadog:` top-level key, or files containing `ad.datadoghq.com` / `prometheus.io/scrape` annotations, or `ServiceMonitor` / `PodMonitor` / `PrometheusRule` resources); OR the diff contains a `HorizontalPodAutoscaler` or `VerticalPodAutoscaler` resource; OR the diff contains a `PersistentVolumeClaim` resource or a `volumeClaimTemplates` section; OR the diff adds or modifies a `Service` with `type: LoadBalancer`.
