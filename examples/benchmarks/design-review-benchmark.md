...
**Date:** 2026-04-07
**Skill:** `kk:design-review` (standard mode)
...
**Prompt:** `/kk:design-review language-specific-skills`
...
**Prompt:** `/kk:design-review synthetic-flawed-feature`
...
- **[MISSING]** `design-review` skill omitted from design and implementation docs
  - **Confidence:** 9/10 -- Verified by searching all three docs. Zero matches. Yet the
    codebase shows `design-review/SKILL.md` and `review-process.md` both have full capy
    integration. The design docs are incomplete -- they don't document a feature that was
    actually shipped.
  - **Evidence:** `design-review/SKILL.md` line 12: protocol reference present.
...
- Found a **P1 finding the baseline missed entirely**: the `design-review` skill was shipped with capy integration but never documented in the design/implementation docs — a real undocumented feature
...
The `kk:design-review` skill's value proposition is **output standardization, not issue detection**. Claude already catches design flaws effectively without skill guidance. The skill ensures:
