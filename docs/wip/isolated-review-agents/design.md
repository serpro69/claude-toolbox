...
- **Main agent** (runs `pal codereview` with the top available pal model): External model, naturally isolated. Receives git diff. Produces findings in its own format.
