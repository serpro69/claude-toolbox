...
- [ ] Verify: both files render as valid Markdown; internal links resolve; no stale references to the old `review-code/reference/<lang>/` path anywhere in `CLAUDE.md` or `README.md`. When documenting the `${CLAUDE_PLUGIN_ROOT}` convention in CLAUDE.md, use alternative quoting (fenced code block or escape) — inline backticks do NOT prevent expansion, per the verification in ADR 0003.
...
- [ ] **document**: review `CLAUDE.md` for accuracy with all phases landed. Confirm the `${CLAUDE_PLUGIN_ROOT}` convention references used alternative quoting (fenced code blocks or escapes) where documenting the variable literally, per the ADR 0003 inline-code-span gotcha.
