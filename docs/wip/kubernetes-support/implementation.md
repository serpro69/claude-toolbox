...
2. **Per-consumer input model.** Enumerate what each consuming skill passes as detection input:
   ...
   - `design` → user-declared signal OR keyword inference from the idea prose. Detection in the design phase is not file-based. The file must spell out the interaction: "check idea prose for K8s-shaped keywords; if any match, ask the user to confirm activation; if no match but idea is ambiguous, ask explicitly."
