...
Invoke the `kk:implementation-review` skill using the `review-isolated.md` workflow:
...
4. **Present Report** — Organized by finding type, ask user how to proceed (or feed back into implementation-process if mid-feature)

## Examples

Review a specific feature:
```
/kk:implementation-review:isolated jwt-auth
```

Review when only one WIP feature exists:
```
/kk:implementation-review:isolated
```
