...
Invoke the `kk:cove` skill using the `cove-isolated.md` workflow:
...
```
/kk:cove:cove-isolated What is the default port for PostgreSQL?
```

Codebase verification with Explore agent:
```
/kk:cove:cove-isolated --explore How does the auth middleware work in this project?
```

Faster verification with haiku model:
```
/kk:cove:cove-isolated --haiku What is the speed of light in m/s?
```

Custom agent type:
```
/kk:cove:cove-isolated --agent=general-purpose What is the capital of France?
```

Combined flags:
```
/kk:cove:cove-isolated --haiku --explore What testing pattern does this codebase use?
```

Verify previous response:
```
/kk:cove:cove-isolated
```
