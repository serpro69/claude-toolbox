...
- **Artifact**: the path to the single accepted artifact under review (and, for a design doc, the heading scoping the review).
...
Grade the artifact's reasoning against its own stated context per the soundness procedure (appropriateness + reversibility). Internal-soundness only — no reality branch.
...
- `delegated` and `unrouted` claims always appear under Not Reviewed.
- Do NOT include a "next steps" section — the spawning skill handles that.
